...
//   }
	...
	@Override
	public PageNavigation makePageNavigation(int currentPage, int sizePerPage, String searchWord) throws SQLException {
		PageNavigation pageNavigation = new PageNavigation();
		int naviSize = 10;// 페이지 갯수
		pageNavigation.setCurrentPage(currentPage);
		pageNavigation.setNaviSize(naviSize);
		int totalCount = 0;
		if(searchWord == null) totalCount = dao.getTotalCount();// 총 개시글수
		else totalCount = dao.getTotalCountBySearchWord(searchWord);
	...
	@Override
	public List<HouseDeal> detaillist(int cp, int spp, String searchWord) throws SQLException {
		Trio t = new Trio();
		t.setX(cp);
		t.setY(spp);
		t.setStr(searchWord);
		return dao.detaillist(t);
	}
