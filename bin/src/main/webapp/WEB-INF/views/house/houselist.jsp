...
<link rel="stylesheet" type="text/css" href=<c:url value="/house/houselist.css"/> />
</head>
<body>
	<%@ include file="/WEB-INF/views/header.jsp"%>
	<form name="pageform" id="pageform" method="GET" action="">
		<input type="hidden" name="act" id="act" value="houselist"> 
		<input type="hidden" name="pg" id="pg" value="">
	</form>
	<h1 id = "mainWindow" align="center">전체 검색 화면</h1>
	<form id="searchform" method="post" action="<c:url value="/house.do"/>">
		<input type="hidden" name="act" value="detaillist" /> <input
			type="hidden" name="pg" value="1" />
		<div align="left">
			<label>
			<input type="checkbox" name="type1" value="1">
				아파트 매매</label> <label><input type="checkbox" name="type2" value="2">
		...
		<button class="btn btn-secondary" type="button" onclick="javascript:search();">검색</button>
		</div>
	</form>
		...
		<thead class="thead-light">
			...
			<c:forEach var="house" items="${List}">
				<tr>
					...
					<td>${house.dong}</td>
					<td><a
						href="<c:url value="//house.do?act=detaillistByNo&no=${house.no}"/>">${house.aptName}</a>
					</td>
